...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, waggoxd):
        golist = waggoxd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_waggoxd(waggoxd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_waggoxd(self, waggoxd, rec):
        import decimal
        import waggoxlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from waggoxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from waggoxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from waggoxd - Waggoxd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(waggoxd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from waggoxd! %s" % e)
            govobj.vote_delete(waggoxd)
            return (govobj, None)
    ...
    def vote_delete(self, waggoxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(waggoxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, waggoxd, signal, outcome):
        import waggoxlib
        ...
        output = waggoxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = waggoxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(waggoxd, signal)

    def sync_network_vote(self, waggoxd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = waggoxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = WAGGOXD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 waggox addr, non-multisig
            if not waggoxlib.is_valid_waggox_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Waggox address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Waggox Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (waggoxlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = WAGGOXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not waggoxlib.is_valid_waggox_address(addr, config.network):
    ...
    def hash(self):
        import waggoxlib
        return waggoxlib.hashit(self.serialise())
