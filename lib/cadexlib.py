...
def is_valid_waggox_address(address, network='mainnet'):
    ...
    waggox_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Waggox address
        return False

    if (address_version != waggox_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('waggox.conf')
