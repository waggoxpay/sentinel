...
class WaggoxDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some waggoxd methods
    ...
    @classmethod
    def from_waggox_conf(self, waggox_dot_conf):
        from waggox_config import WaggoxConfig
        config_text = WaggoxConfig.slurp_config_file(waggox_dot_conf)
        creds = WaggoxConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = waggoxlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = waggoxlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = waggoxlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_waggoxd_proto_version_with_sentinel_ping)
