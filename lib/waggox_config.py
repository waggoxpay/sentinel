...
class WaggoxConfig():
    ...
    def slurp_config_file(self, filename):
        # read waggox.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is waggox.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from waggox.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Waggox defaults...
        default_port = 9920 if (network == 'mainnet') else 9921

        # use default port for network if not specified in waggox.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from waggox.conf
