...
def has_waggox_conf():
    ...
    valid_waggox_conf = False

    # ensure waggox_conf exists & readable
    #
    # if not, print a message stating that Waggox Core must be installed and
    # configured, including JSONRPC access in waggox.conf
    try:
        f = io.open(config.waggox_conf)
        valid_waggox_conf = True
    except IOError as e:
        print(e)

    return valid_waggox_conf
...
def main():
    ...
    if not has_waggox_conf():
        print("WaggoxCore must be installed and configured, including JSONRPC access in waggox.conf")
        sys.exit(1)
