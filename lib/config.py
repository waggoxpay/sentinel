...
sentinel_cfg = WaggoxConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_waggoxd_proto_version_with_sentinel_ping = 70207


def get_waggox_conf():
    if sys.platform == 'win32':
        waggox_conf = os.path.join(os.getenv('APPDATA'), "WAGGOXCOIN/waggox.conf")
    else:
        home = os.environ.get('HOME')

        waggox_conf = os.path.join(home, ".WAGGOXCOIN/waggox.conf")
        if sys.platform == 'darwin':
            waggox_conf = os.path.join(home, "Library/Application Support/WAGGOXCOIN/waggox.conf")

    waggox_conf = sentinel_cfg.get('waggox_conf', waggox_conf)
